...
$sql="SELECT * from `payment_bankaccount` where active=1 ";
$result=$connection2->prepare($sql);
$result->execute();
$bankresult=$result->fetchAll();
...
<tr style="height:100px;">
	...
	<td><b>Payment Mode:</b>
		<select name='p_mode' id='p_mode'>
			...
			<option value='bank_transfer' <?=$p_mode=='bank_transfer'?'selected':'';?>>Bank Transfer</option>
			<option value='dc_card' <?=$p_mode=='dc_card'?'selected':'';?>>Dr/Cr Card</option>
		</select>
	</td>
	
	<td><b>Bank Mode:</b>
		<select name='b_mode' id='b_mode'>
			<option value='1' <?=$b_mode=='1'?'selected':'';?>>Canara Bank</option>
			<option value='3' <?=$b_mode=='3'?'selected':'';?>>S.B.I</option>
		</select>
	</td>
...
echo $bank;
 ...
 ?>
	...
	<tr>
	<td><input type="submit" name="submit" id="submit" value="Go" style=""></td>
	<?php if($_POST){?><td><a type="button" name="print_headwise" id="print_headwise"  style="color: #ffffff; background-color: #ff731b; padding:5px 10px;pointer:cursor;">Print</a></td> <?php } ?>
