<?php 
@session_start() ;
if (isActionAccessible($guid, $connection2, "/modules/Fee/collection_summary.php")==FALSE) {
	//Acess denied
...
try {
		...
		WHERE `payment_staus` =  'Paid' AND  `payment_date` <=  '".$src_end_date."' AND (".$query_mont_condition_str.") ";
		if($class!='')
			$sql_condition.=" AND gibbonyeargroup.gibbonYearGroupID=".$class;
		if($section!='')
			$sql_condition.=" AND gibbonrollgroup.gibbonRollGroupID=".$section;
		...
		WHERE (".$query_transport_mont_str.") AND ( transport_month_entry.payment_master_id>0 AND  payment_master.`payment_date` <=  '".$src_end_date."') AND `transport_month_entry`.`gibbonSchoolYearID` =$year";
		if($left_student=='')
...
?>
<form name="collection_form" id="collection_form" method="post" action="<?php print $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . $_SESSION[$guid]["module"] . "/collection_summary.php" ?>">
...
<?php } ?></b>
  ...
  <tr>
    ...
    <td >
	    ...
	    <input type="button" id="collection_print_c" name="collection_print_c" value="Print" style="float: right;">
	    <?php } ?>
	    </td>
  </tr>
</table>
...
<input type="hidden" name="print_page_url" id="print_page_url" value="<?php echo $_SESSION[$guid]["absoluteURL"] . "/modules/Fee/collection_summary_print.php";?>">
