...
$month_sequence=array('Yearly'=>0,'January'=>1,'February'=>2,'March'=>3,'April'=>4,'May'=>5,'June'=>6,'July'=>7,'August'=>8,'September'=>90,'October'=>91,'November'=>92,'December'=>93);
if($_POST)
{
	...
	{	
		...
		$sql="SELECT fee_payable.*,fee_rule_master.rule_name,fee_type_master.fee_type_name,fee_type_master.fee_type_master_id, gibbonschoolyear.name AS year,cheque_master.cheque_status_id
		 FROM fee_payable 
		 LEFT JOIN cheque_master ON cheque_master.payment_master_id=fee_payable.payment_master_id
		...
		$query="SELECT `transport_month_entry`.`transport_month_entryid`,month_name,price,gibbonschoolyear.name AS year,transport_month_entry.payment_master_id AS payment_master_id,payment_master.voucher_number AS v_no,cheque_master.cheque_status_id
		...
		LEFT JOIN cheque_master ON cheque_master.payment_master_id=payment_master.payment_master_id
		...
		{
			if($_REQUEST['payment_status_filter']!='cancelled'){
			...
			$query.=" AND transport_month_entry.payment_master_id".$condition."0";
				...
				$sql.=" AND payment_staus='paid' AND `cheque_status_id`=0";
				$query.=" AND transport_month_entry.payment_master_id>0 AND `cheque_status_id`=0";
			}
		}
...
<?php foreach ($all_fee_type as $value) { 
       ...
       //	if($value['boarder']=='yes')$boarder='Boarder';else $boarder='Non-Boarder';
       	...
       	?>
       	   <option value="<?php echo $value['fee_type_master_id']?>" <?php if($fee_type==$value['fee_type_master_id']){?> selected="selected"<?php } ?>><?php echo $value['fee_type_name']?></option>
      <?php } ?> 
...
<option value="cancelled" <?php if($payment_status=='cancelled'){?> selected="selected"<?php } ?>>Cancelled</option>
</select>
</td>
...
<tbody> 
  ...
  <tr <?php if($value['payment_staus']=='paid' && $value['cheque_status_id']=='0'){echo "style='color:darkred'";}else if($value['payment_staus']=='paid'){echo "style='color:green'";}else{ echo 'style="color:red"';}?>>
    ...
    <td><?php
    	...
    	if($value['payment_staus']=='paid' && $value['cheque_status_id']=='0'){
			echo "Cancelled - ".$value['voucher_number'];
		}
		else if($value['payment_staus']=='paid'){
  ...
  ?>
	<tr <?php if($a['payment_master_id']>0 && $a['cheque_status_id']=='0'){echo "style='color:darkred'";}else if($a['payment_master_id']>0){echo "style='color:green'";}else{ echo 'style="color:red"';}?>>
	...
	<?php
		if($a['payment_master_id']>0 && $a['cheque_status_id']=='0'){
			echo "<td style='font-size:12px'>Cancelled - ".$a['v_no']."</td>";
		}
		else if($a['payment_master_id']>0){
			echo "<td>Paid - ".$a['v_no']."</td>";
		}
		else{
			echo "<td>Unpaid"."</td>";
		}
	 ...
	 $(document).ready(function(){
		$('.myTable').DataTable({
		 ...
		 $('.feeDelete').click(function(){
			 var id=$(this).attr('id');
			 var proceed=confirm("Are you sure you want to delete this fee?");
			 if(!proceed)
				 return false;
			$.ajax
	 		({
