...
if(!$_POST){
	...
	print "<h3>RE-ENROLMENT OF OLD STUDENTS</h3>";
						try {
							...
							$sqlReenrol="SELECT gibbonperson.gibbonPersonID, account_number, preferredName, `gibbonyeargroup`.`sequenceNumber`,`gibbonyeargroup`.`name` as class,`gibbonrollgroup`.`name` as section, `gibbonstudentenrolment`.`rollOrder`,gibbonRollGroupIDNext 
								...
								AND `gibbonperson`.`gibbonPersonID` NOT IN (SELECT `student_id` FROM `leftstudenttracker` WHERE 1) AND `gibbonyeargroup`.`gibbonYearGroupID` NOT IN (15,16,17,24,25,26,27,28)
	...
	print "<table width='100%'>";
		...
		print "</tr>";
	foreach($data as $d){
		$sqlSection="SELECT `gibbonRollGroupID`,`gibbonrollgroup`.`name`FROM `gibbonrollgroup`  JOIN gibbonyeargroup ON gibbonyeargroup.gibbonYearGroupID=gibbonrollgroup.gibbonYearGroupID WHERE `gibbonSchoolYearID`=$nextYearID AND sequenceNumber=".$d['sequenceNumber'];
