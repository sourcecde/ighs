...
if (isActionAccessible($guid, $connection2, "/modules/Staff/staff_view_details.php")==FALSE) {
	...
	if(isset($_REQUEST['type'])){
		...
		$sql2="SELECT * FROM `gibbonschoolyear` ORDER BY `gibbonSchoolYearID` ";
...
<?php
	...
	//	$sql="SELECT gibbonstaff.gibbonStaffID,gibbonstaff.type,gibbonstaff.bank_ac,gibbonstaff.preferredName,gibbonrollgroup.name as section FROM gibbonstaff
	...
	//	$sql.=" ORDER BY gibbonstaff.priority, gibbonstaff.category,gibbonrollgroup.section DESC";
		...
		$sql="SELECT gibbonstaff.gibbonStaffID,gibbonstaff.type,gibbonstaff.bank_ac,gibbonstaff.preferredName FROM gibbonstaff
				 WHERE payment_mode='{$_REQUEST['type']}' 
				 AND sec_code = {$staff_type}";
		$sql.=" ORDER BY gibbonstaff.priority, gibbonstaff.category DESC";
