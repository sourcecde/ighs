...
if (isActionAccessible($guid, $connection2, "/modules/School Admin/house_manage.php")==FALSE) {
	...
	print "<div class='trailHead'><a href='" . $_SESSION[$guid]["absoluteURL"] . "'>" . _("Home") . "</a> > <a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . getModuleName($_GET["q"]) . "/" . getModuleEntry($_GET["q"], $connection2, $guid) . "'>" . _(getModuleName($_GET["q"])) . "</a> > </div><div class='trailEnd'>" . _('View Houses') . "</div>" ;
	print "</div>" ;
	...
	try {
		...
		/*
		$sql="SELECT gibbonperson.* from gibbonperson where gibbonHouseID=:gibbonHouseID AND dateEnd IS NULL 
