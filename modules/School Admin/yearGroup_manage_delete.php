...
if (isActionAccessible($guid, $connection2, "/modules/School Admin/yearGroup_manage_delete.php")==FALSE) {
	...
	print "<div class='trailHead'><a href='" . $_SESSION[$guid]["absoluteURL"] . "'>" . _("Home") . "</a> > <a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . getModuleName($_GET["q"]) . "/" . getModuleEntry($_GET["q"], $connection2, $guid) . "'>" . _(getModuleName($_GET["q"])) . "</a> > <a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . getModuleName($_GET["q"]) . "/yearGroup_manage.php'>" . _('Manage Year Groups') . "</a> > </div><div class='trailEnd'>" . _('Delete Year Group') . "</div>" ;
	print "</div>" ;
	...
	if ($gibbonYearGroupID=="") {
		...
		if ($result->rowCount()!=1) {
			...
			$row=$result->fetch() ;
			?>
			<form method="post" action="<?php print $_SESSION[$guid]["absoluteURL"] . "/modules/" . $_SESSION[$guid]["module"] . "/yearGroup_manage_deleteProcess.php?gibbonYearGroupID=$gibbonYearGroupID" ?>">
				<table class='smallIntBorder' class='smallIntBorder' cellspacing='0' style="width: 100%">	
					<tr>
						<td> 
