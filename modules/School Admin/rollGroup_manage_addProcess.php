...
if (isActionAccessible($guid, $connection2, "/modules/School Admin/rollGroup_manage_add.php")==FALSE) {
	...
	if ($gibbonSchoolYearID=="" OR $name=="" OR $nameShort=="") {
		...
		try {
			$data=array("name"=>$name, "nameShort"=>$nameShort, "gibbonSchoolYearID"=>$gibbonSchoolYearID,"gibbonYearGroupID"=>$gibbonYearGroupID); 
			$sql="SELECT * FROM gibbonrollgroup WHERE (name=:name OR nameShort=:nameShort) AND gibbonSchoolYearID=:gibbonSchoolYearID AND gibbonYearGroupID=:gibbonYearGroupID" ;
