...
if (isActionAccessible($guid, $connection2, "/modules/School Admin/rollGroup_manage_delete.php")==FALSE) {
	...
	if ($gibbonRollGroupID=="" AND $gibbonSchoolYearID=="") {
		...
		if ($result->rowCount()!=1) {
			...
			$row=$result->fetch() ;
			?>
			<form method="post" action="<?php print $_SESSION[$guid]["absoluteURL"] . "/modules/" . $_SESSION[$guid]["module"] . "/rollGroup_manage_deleteProcess.php?gibbonRollGroupID=$gibbonRollGroupID" ?>">
