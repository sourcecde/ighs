...
if (isActionAccessible($guid, $connection2, "/modules/School Admin/gradeScales_manage_delete.php")==FALSE) {
	...
	if ($gibbonScaleID=="") {
		...
		if ($result->rowCount()!=1) {
			...
			$row=$result->fetch() ;
			?>
			<form method="post" action="<?php print $_SESSION[$guid]["absoluteURL"] . "/modules/" . $_SESSION[$guid]["module"] . "/gradeScales_manage_deleteProcess.php?gibbonScaleID=$gibbonScaleID" ?>">
