...
if (isActionAccessible($guid, $connection2, "/modules/School Admin/inDescriptors_manage_add.php")==FALSE) {
	...
	if ($name=="" OR $nameShort=="" OR $sequenceNumber=="") {
		...
		try {
			...
			$sql="SELECT * FROM gibbonindescriptor WHERE name=:name OR nameShort=:nameShort OR sequenceNumber=:sequenceNumber" ;
		...
		if ($result->rowCount()>0) {
			...
			try {
				...
				$sql="INSERT INTO gibbonindescriptor SET name=:name, nameShort=:nameShort, sequenceNumber=:sequenceNumber, description=:description" ;
				$result=$connection2->prepare($sql);
				$result->execute($data);
			}
			catch(PDOException $e) { 
