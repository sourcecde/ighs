...
if (isActionAccessible($guid, $connection2, "/modules/School Admin/rollGroup_manage.php")==FALSE) {
	...
	print "<div class='trailHead'><a href='" . $_SESSION[$guid]["absoluteURL"] . "'>" . _("Home") . "</a> > <a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . getModuleName($_GET["q"]) . "/" . getModuleEntry($_GET["q"], $connection2, $guid) . "'>" . _(getModuleName($_GET["q"])) . "</a> > </div><div class='trailEnd'>" . _('Manage Roll Groups') . "</div>" ;
	print "</div>" ;
	...
	if ($gibbonSchoolYearID!="") {
		...
		try {
			...
			$sql="SELECT gibbonschoolyear.gibbonSchoolYearID, gibbonRollGroupID, gibbonschoolyear.name as yearName,gibbonyeargroup.name as class,gibbonyeargroup.nameShort as classShort, gibbonrollgroup.name, gibbonrollgroup.nameShort, gibbonPersonIDTutor, gibbonPersonIDTutor2, gibbonPersonIDTutor3, gibbonspace.name AS space, website FROM gibbonrollgroup JOIN gibbonschoolyear ON (gibbonrollgroup.gibbonSchoolYearID=gibbonschoolyear.gibbonSchoolYearID) LEFT JOIN gibbonspace ON (gibbonrollgroup.gibbonSpaceID=gibbonspace.gibbonSpaceID) JOIN gibbonyeargroup ON gibbonyeargroup.gibbonYearGroupID=gibbonrollgroup.gibbonYearGroupID WHERE gibbonschoolyear.gibbonSchoolYearID=:gibbonSchoolYearID ORDER BY gibbonyeargroup.sequenceNumber, gibbonrollgroup.name" ; 
			$result=$connection2->prepare($sql);
			$result->execute($data);
		}
		catch(PDOException $e) { 
			...
			print "<table cellspacing='0' style='width: 100%'>" ;
				...
				while ($row=$result->fetch()) {
					...
					print "<tr class=$rowNum>" ;
						print "<td>" ;
							print "<b>" .$row['class']." ".$row["name"] ."</b><br/>" ;
							print "<span style='font-size: 85%; font-style: italic'>" . $row["classShort"]." ".$row["nameShort"] . "</span>" ;
						print "</td>" ;
						print "<td>" ;
							try {
						...
						print "<td>" ;
							...
							print "<a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . $_SESSION[$guid]["module"] . "/rollGroup_manage_delete.php&gibbonRollGroupID=" . $row["gibbonRollGroupID"] . "&gibbonSchoolYearID=$gibbonSchoolYearID'><img title='" . _('Delete') . "' src='./themes/" . $_SESSION[$guid]["gibbonThemeName"] . "/img/garbage.png'/></a> " ;
						print "</td>" ;
					print "</tr>" ;
