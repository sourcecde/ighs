...
if (isActionAccessible($guid, $connection2, "/modules/School Admin/house_manage_add.php")==FALSE) {
	...
	if ($name=="" OR $nameShort=="") {
		...
		if ($result->rowCount()>0) {
			...
			//Write to database
			try {
				$data=array("name"=>$name, "nameShort"=>$nameShort,"student_personid"=>$student_personid,"stuff_personid"=>$stuff_personid); 
				$sql="INSERT INTO gibbonhouse SET name=:name, nameShort=:nameShort,student_personid=:student_personid,stuff_personid=:stuff_personid" ;
				$result=$connection2->prepare($sql);
				$result->execute($data);
			}
			catch(PDOException $e) { 
