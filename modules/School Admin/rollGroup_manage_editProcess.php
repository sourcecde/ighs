...
if (isActionAccessible($guid, $connection2, "/modules/School Admin/rollGroup_manage_edit.php")==FALSE) {
	...
	if ($gibbonRollGroupID=="" OR $gibbonSchoolYearID=="") {
		...
		try {
			...
			echo $sql="SELECT * FROM gibbonrollgroup WHERE gibbonRollGroupID=$gibbonRollGroupID AND gibbonSchoolYearID=$gibbonSchoolYearID" ;
		...
		if ($result->rowCount()!=1) {
			...
			$gibbonYearGroupID=$_POST["gibbonyeargroup"];
			...
			if ($gibbonSchoolYearID=="" OR $name=="" OR $nameShort=="") {
				...
				try {
					$data=array("name"=>$name, "nameShort"=>$nameShort, "gibbonRollGroupID"=>$gibbonRollGroupID, "gibbonSchoolYearID"=>$gibbonSchoolYearID,"gibbonYearGroupID"=>$gibbonYearGroupID); 
					$sql="SELECT * FROM gibbonrollgroup WHERE (name=:name OR nameShort=:nameShort) AND NOT gibbonRollGroupID=:gibbonRollGroupID AND gibbonSchoolYearID=:gibbonSchoolYearID AND gibbonYearGroupID=:gibbonYearGroupID" ;
					$result=$connection2->prepare($sql);
					$result->execute($data);
				}
				catch(PDOException $e) { 
