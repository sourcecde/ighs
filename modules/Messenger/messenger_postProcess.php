						...
						foreach ($choices as $t) {
							...
							if ($sms=="Y" AND $countryCode!="") {
								if ($staff=="Y") {
									...
									while ($rowEmail=$resultEmail->fetch()) {
										$phones.=$rowEmail["phone"] . "," ; $phonesReport.=formatName('', $rowEmail["preferredName"], $rowEmail["surname"], "Student", false) . " (" . $countryCode . $rowEmail["phone"] . ")," ;
										/*
										if ($rowEmail["countryCode"]=="") {
											$phones.=$countryCode . $rowEmail["phone"] . "," ; $phonesReport.=formatName('', $rowEmail["preferredName"], $rowEmail["surname"], "Student", false) . " (" . $countryCode . $rowEmail["phone"] . ")," ;
										}
					...
					if ($smsUsername!="" AND $smsPassword!="" AND $smsURL!="") {
						...
						for ($i=0; $i<$sendReps; $i++) {
							...
							for ($n=0; $n<$numbersPerSend; $n++) {
								...
								$numCache=substr($numCache,0,-1);
							//echo $number_count;
							if($number_count>2)
								$numCache="(".$numCache.")";
							////////// Test
							
							$query="?user=$smsUsername:$smsPassword&senderID=TEST SMS&receipientno=".$numCache."&dcs=0&msgtxt=". rawurlencode(stripslashes(strip_tags($body))) ."&state=3" ;        
							//$query="?user=$smsUsername:$smsPassword&senderID=". rawurlencode($_SESSION[$guid]["organisationNameShort"]) ."&receipientno=". rawurlencode(substr($numCache,0,-1)) ."&dcs=0&msgtxt=". rawurlencode(stripslashes(strip_tags($body))) ."&state=4" ;        
							...
							/*For sending SMS 
									...
									curl_setopt($curlSession, CURLOPT_URL, $smsURL. $query);
									curl_setopt($curlSession, CURLOPT_BINARYTRANSFER, true);
									curl_setopt($curlSession, CURLOPT_RETURNTRANSFER, true);
									$result=curl_exec($curlSession);
										//$result=explode("is",$Result);
