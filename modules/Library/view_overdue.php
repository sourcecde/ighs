...
<?php
		try{
		$today=date('Y-m-d');
		$sql="SELECT `borrowID`,`dateBorrow`,`dateDue`,`lakshya_library_bookmaster`.`bookID`,`lakshya_library_bookmaster`.`acc_no`,`lakshya_library_booknamemaster`.`title`,`lakshya_library_booknamemaster`.`author`,   
				...
				WHERE `borrowStatus`='Pending' AND `dateDue` < '$today'";
		if($_POST){
			if($section!='')
		...
		foreach($data as $d){
			echo "<tr>";
				...
				echo "<td>{$d['title']}<br><span style='float: right'><b>Author: </b>{$d['author']}</span></td>";
					$accNo=$d['account_number']+0;
				echo "<td>{$d['preferredName']}&nbsp; &nbsp; ($accNo)<br><span style='float: left'><b>Class:</b> {$d['section']}</span><span style='float: right'><b>Roll:</b> {$d['rollOrder']}</span></td>";
					$dB=dateFormatR($d['dateBorrow']);
					$dD=dateFormatR($d['dateDue']);
				echo "<td>$dB</td>";
				echo "<td>$dD</td>";
				//echo "<td><button class='cButton returnB' id='{$d['borrowID']}_{$d['bookID']}'>Return</button></td>";
			echo "</tr>";
		}
	}
	else
...
function dateFormatR($oDate){
 ...
 var bookSearchURL=$('#bookSearchURL').val();
	...
	$('#cReturnB').click(function(){
		var id=$('#rID').val();
		var returnDate=$('#returnDate').val();
		$.ajax({
			type: 'POST',
			url: bookSearchURL,
			data: {action: 'returnBook',returnDate: returnDate, id: id},
			success: function(msg){
				console.log(msg);
