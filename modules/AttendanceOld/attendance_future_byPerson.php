				...
				<td class="right">
					<select style="width: 302px" name="gibbonPersonID">
						...
						try {
							...
							echo $sqlSelect="SELECT * FROM gibbonperson							WHERE (dateStart IS NULL OR dateStart<='" . date("Y-m-d") . "') AND (dateEnd IS NULL  OR dateEnd>='" . date("Y-m-d") . "') ORDER BY officialName" ;
						...
						while ($rowSelect=$resultSelect->fetch()) {
							if ($gibbonPersonID==$rowSelect["gibbonPersonID"]) {
								print "<option selected value='" . $rowSelect["gibbonPersonID"] . "'>" . formatName("", htmlPrep($rowSelect["preferredName"]), htmlPrep($rowSelect["surname"]), "Student", true) . " (" . htmlPrep($rowSelect["account_number"]) . ")</option>" ;
								//print "<option selected value='" . $rowSelect["gibbonPersonID"] . "'>" . htmlPrep($rowSelect["officialName"]). " (" . htmlPrep($rowSelect["nameShort"]) . ")"."</option>" ;
							}
							else {
								print "<option value='" . $rowSelect["gibbonPersonID"] . "'>" . formatName("", htmlPrep($rowSelect["preferredName"]), htmlPrep($rowSelect["surname"]), "Student", true) . " (" . htmlPrep($rowSelect["account_number"]) . ")</option>" ;
								//print "<option selected value='" . $rowSelect["gibbonPersonID"] . "'>" . htmlPrep($rowSelect["officialName"]). " (" . htmlPrep($rowSelect["nameShort"]) . ")"."</option>" ;
							}
						}
						?>				
