...
if (isActionAccessible($guid, $connection2, "/modules/User Admin/user_manage_edit.php")==FALSE) {
	...
	if ($gibbonPersonID=="") {
		...
		if ($result->rowCount()!=1) {
			...
			$nationalIDCardScan='';
			if(isset($_POST["nationalIDCardScanCurrent"]))
			{
				$nationalIDCardScan=$_POST["nationalIDCardScanCurrent"];
			}
			$citizenship1PassportScan='';
			if(isset($_POST["citizenship1PassportScanCurrent"]))
			{
			...
			$preferredName=$firstName.' '.$surname;
			/*
			...
			*/
			...
			$website='http://ezytechno.com/gibbon';
			$languageFirst=$_POST["languageFirst"] ;
			$languageSecond=$_POST["languageSecond"] ;
			$languageThird='';
			$account_number=isset($_POST["account_number"])?$_POST["account_number"]:'';
			$admission_number=isset($_POST["admission_number"])?$_POST["admission_number"]:'';
			$rollgroup=isset($_POST["rollgroup"])?$_POST["rollgroup"]:'';
			$rollOrder=isset($_POST["rollOrder"])?$_POST["rollOrder"]:'';
	...
	if(isset($_POST["visaExpiryDate"]))
	{
		$visaExpiryDate=$_POST["visaExpiryDate"] ;
			...
			//Validate Inputs
			if ($surname=="" OR $firstName==""  OR $officialName=="" OR $gender=="" OR $username=="" OR $status=="" OR $gibbonRoleIDPrimary=="") {
				//Fail 3
				...
				if ($result->rowCount()>0) {
					...
					if ($_FILES['file1']["tmp_name"]!="" OR $_FILES['file2']["tmp_name"]!="") {
						...
						if ($_FILES['file1']["tmp_name"]!="") {
							...
							if (!(move_uploaded_file($_FILES["file1"]["tmp_name"],"../../" . $attachment1))) {
								$attachment1="" ;
								$imageFail=TRUE ;
						...
						if ($_FILES['file2']["tmp_name"]!="") {
							...
							if (!(move_uploaded_file($_FILES["file2"]["tmp_name"],"../../"  . $attachment2))) {
								$attachment2="" ;
								$imageFail=TRUE ;
							}
						}
						...
						*/
						//Check image sizes
						if ($attachment1!="") {
							$size1=getimagesize("../../" . $attachment1) ;
							$width1=$size1[0] ;
							$height1=$size1[1] ;
						...
						if ($attachment2!="") {
							$size2=getimagesize("../../" . $attachment2) ;
							$width2=$size2[0] ;
							$height2=$size2[1] ;
					...
					try{
						$data=array("gibbonPersonID"=>$gibbonPersonID,"gibbonSchoolYearID"=>$_SESSION[$guid]['gibbonSchoolYearIDCurrent'],"gibbonRollGroupID"=>$rollgroup,"rollOrder"=>$rollOrder);
						$sql="UPDATE gibbonstudentenrolment SET gibbonRollGroupID=:gibbonRollGroupID,rollOrder=:rollOrder WHERE gibbonPersonID=:gibbonPersonID AND gibbonSchoolYearID=:gibbonSchoolYearID";
						$result=$connection2->prepare($sql);
						$result->execute($data);
					}
					catch(PDOException $e){
		...
		echo $isLeft;
			...
			{	
				...
				try {
				$data=array("studentId"=>$gibbonPersonID,"studentName"=>$preferredName, "isLeft"=>$isLeft,"yearOfLeaving"=>$yearOfLeaving,"leavingReason"=>$leavingReason, "hasTc"=>$hasTc, "dateOfTc"=>$dateOfTc, "tcNumber"=>$tcNumber);	
				$query="SELECT * FROM leftstudenttracker where student_id=".$gibbonPersonID;
				$result=$connection2->prepare($query);
						$result->execute();
						if ($result->rowCount()!=1)
						{
							$query1="INSERT INTO leftstudenttracker(student_id,studentName,isLeft,yearOfLeaving,leavingReason,hasTc,tcNumber,dateOfTc) 
								values(\"".$gibbonPersonID."\",\"".$preferredName."\",\"".$isLeft."\",\"".$yearOfLeaving."\",\"".$leavingReason."\",\"".$hasTc."\",\"".$dateOfTc."\",\"".$tcNumber."\");";
							...
							$query2="UPDATE leftstudenttracker SET studentName=:studentName, isLeft=:isLeft, yearOfLeaving=:yearOfLeaving, leavingReason=:leavingReason,hasTc=:hasTc, dateOfTc=:dateOfTc, tcNumber=:tcNumber Where student_id=:studentId;";
