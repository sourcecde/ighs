...
if($_POST){
	...
	$sql1="SELECT * FROM gibbonyeargroup WHERE gibbonYearGroupID={$enrolmentData['gibbonYearGroupID']}" ;
	$result1=$connection2->prepare($sql1);
	$result1->execute();
	$class=$result1->fetch();
	
	$sql2="SELECT `fee_boarder_class_id` FROM fee_boarder_class WHERE class='{$class['name']}' AND border='$next_boarder_type'";
	...
	$sql5="DELETE FROM `fee_payable` WHERE `gibbonStudentEnrolmentID`={$enrolmentData['gibbonStudentEnrolmentID']} AND `month_no` IN ($months)";
...
$URL=$_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . $_SESSION[$guid]["module"] . "/change_boarder_type.php";
header("Location: {$URL}");		
?>