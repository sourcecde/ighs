...
if (isActionAccessible($guid, $connection2, "/modules/User Admin/data_finance_delete.php")==FALSE) {
	...
	if ($gibbonFinanceInvoiceeUpdateID=="") {
		...
		if ($result->rowCount()!=1) {
			...
			$row=$result->fetch() ;
			?>
			<form method="post" action="<?php print $_SESSION[$guid]["absoluteURL"] . "/modules/" . $_SESSION[$guid]["module"] . "/data_finance_deleteProcess.php?gibbonFinanceInvoiceeUpdateID=$gibbonFinanceInvoiceeUpdateID" ?>">
				<table class='smallIntBorder' cellspacing='0' style="width: 100%">	
					...
					<tr>
						<td> 
							<input name="gibbonFinanceInvoiceeUpdateID" id="gibbonFinanceInvoiceeUpdateID" value="<?php print $gibbonFinanceInvoiceeUpdateID ?>" type="hidden">
