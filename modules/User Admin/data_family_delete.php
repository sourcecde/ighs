...
if (isActionAccessible($guid, $connection2, "/modules/User Admin/data_family_delete.php")==FALSE) {
	...
	print "<div class='trailHead'><a href='" . $_SESSION[$guid]["absoluteURL"] . "'>" . _("Home") . "</a> > <a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . getModuleName($_GET["q"]) . "/" . getModuleEntry($_GET["q"], $connection2, $guid) . "'>" . _(getModuleName($_GET["q"])) . "</a> > <a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/User Admin/data_family.php'>" . _('Family Data Updates') . "</a> > </div><div class='trailEnd'>" . _('Delete Request') . "</div>" ;
	print "</div>" ;
	...
	if ($gibbonFamilyUpdateID=="") {
		...
		if ($result->rowCount()!=1) {
			...
			$row=$result->fetch() ;
			?>
			<form method="post" action="<?php print $_SESSION[$guid]["absoluteURL"] . "/modules/" . $_SESSION[$guid]["module"] . "/data_family_deleteProcess.php?gibbonFamilyUpdateID=$gibbonFamilyUpdateID" ?>">
				<table class='smallIntBorder' cellspacing='0' style="width: 100%">	
					...
					<tr>
						<td> 
							<input name="gibbonFamilyUpdateID" id="gibbonFamilyUpdateID" value="<?php print $gibbonFamilyUpdateID ?>" type="hidden">
