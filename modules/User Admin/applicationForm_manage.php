...
if (isActionAccessible($guid, $connection2, "/modules/User Admin/applicationForm_manage.php")==FALSE) {
	...
	print "<div class='trailHead'><a href='" . $_SESSION[$guid]["absoluteURL"] . "'>" . _("Home") . "</a> > <a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . getModuleName($_GET["q"]) . "/" . getModuleEntry($_GET["q"], $connection2, $guid) . "'>" . _(getModuleName($_GET["q"])) . "</a> > </div><div class='trailEnd'>" . _('Manage Application Forms') . "</div>" ;
	print "</div>" ;
	...
	if ($gibbonSchoolYearID!="") {
		...
		print "<div class='linkTop'>" ;
			...
			if (getPreviousSchoolYearID($gibbonSchoolYearID, $connection2)!=FALSE) {
				print "<a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . $_SESSION[$guid]["module"] . "/applicationForm_manage.php&gibbonSchoolYearID=" . getPreviousSchoolYearID($gibbonSchoolYearID, $connection2) . "'>" . _('Previous Year') . "</a> " ;
			}
			else {
			...
			if (getNextSchoolYearID($gibbonSchoolYearID, $connection2)!=FALSE) {
				print "<a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . $_SESSION[$guid]["module"] . "/applicationForm_manage.php&gibbonSchoolYearID=" . getNextSchoolYearID($gibbonSchoolYearID, $connection2) . "'>" . _('Next Year') . "</a> " ;
			}
			else {
				print _("Next Year") . " " ;
			}
				...
				while ($row=$resultPage->fetch()) {
					...
					print "<tr class=$rowNum>" ;
						...
						print "<td>" ;
							if ($row["gibbonFamilyID"]!="") {
								try {
									...
									$sqlFamily2="SELECT officialName,email FROM gibbonfamilyadult  WHERE gibbonFamilyID=:gibbonFamilyID ORDER BY contactPriority, officialName" ;
								...
								$name=$row["parent1title"]." ".$row["parent1firstName"]." ".$row["parent1surname"];
								if ($row["parent1email"]!="") {
								...
								if ($row["parent2surname"]!="" AND $row["parent2preferredName"]!="") {
									...
									$name=$row["parent2title"]." ".$row["parent2firstName"]." ".$row["parent2surname"];
									if ($row["parent2email"]!="") {
						...
						print "<td>" ;
							...
							print "<a href='modules/".$_SESSION[$guid]["module"] ."/print_money_reciept.php?appllicationId=".$row["gibbonApplicationFormID"]."' onclick='printReciept(this.href); return false;'><img title='" . _('Print Money Receipt') . "' src='./themes/" . $_SESSION[$guid]["gibbonThemeName"] . "/img/print.png'/></a>";
						print "</td>" ;
					print "</tr>" ;
				}
			print "</table>" ;
...
<script>
function printReciept(url)
{
	window.open(url, "_blank", "toolbar=yes, scrollbars=yes, resizable=yes, top=500, left=500, width=700, height=500");
}
</script>