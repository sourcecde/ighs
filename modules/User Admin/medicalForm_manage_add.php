				...
				<td class="right">
					<select style="width: 302px" name="gibbonPersonID" id="gibbonPersonID">
						...
						while ($rowSelect=$resultSelect->fetch()) {
							$s=$_REQUEST['gibbonPersonID']==$rowSelect["gibbonPersonID"]?'selected':'';
							print "<option $s value='" . $rowSelect["gibbonPersonID"] . "'>" . formatName("", htmlPrep($rowSelect["preferredName"]), htmlPrep($rowSelect["surname"]), "Student", true) . " (" . htmlPrep($rowSelect["nameShort"]) . ")</option>" ;
						}
						?>					
					</select>
