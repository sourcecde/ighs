...
try {
  	$sql="UPDATE `gibbonperson` SET `admission_number`='$admission' WHERE `gibbonPersonID`=(SELECT `gibbonPersonID` FROM `gibbonstudentenrolment` WHERE `gibbonStudentEnrolmentID`=$enrollID)" ;
	$result=$connection2->prepare($sql);
	$result->execute();
}
catch(PDOException $e) {
