...
if (isActionAccessible($guid, $connection2, "/modules/User Admin/user_manage.php")==FALSE) {
	...
	print "<div class='trailHead'><a href='" . $_SESSION[$guid]["absoluteURL"] . "'>" . _("Home") . "</a> > <a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . getModuleName($_GET["q"]) . "/" . getModuleEntry($_GET["q"], $connection2, $guid) . "'>" . _(getModuleName($_GET["q"])) . "</a> > </div><div class='trailEnd'>" . _('Manage Users') . "</div>" ;
	print "</div>" ;
		...
		<table class='noIntBorder' cellspacing='0' style="width: 100%">	
			...
			<tr>
				<td> 
					...
					<span style="font-size: 90%"><i><?php print _('Preferred, surname, username, email, phone number, vehicle registration') ?></i></span>
				</td>
				<td class="right">
			...
			while ($row=$resultPage->fetch()) {
				...
				print "<tr class=$rowNum>" ;
					...
					print "<td>" ;
						...
						print $row["title"]." ".$row["firstName"]." ".$row["surname"];
					print "</td>" ;
					print "<td>" ;
						print $row["status"] ;
					print "</td>" ;
					...
					print "</td>" ;
					print "<td>" ;
						print "<a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . $_SESSION[$guid]["module"] . "/user_manage_edit.php&gibbonPersonID=" . $row["gibbonPersonID"] . "&search=$search&url=1'><img title='" . _('Edit') . "' src='./themes/" . $_SESSION[$guid]["gibbonThemeName"] . "/img/config.png'/></a> " ;
						print "<a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . $_SESSION[$guid]["module"] . "/user_manage_delete.php&gibbonPersonID=" . $row["gibbonPersonID"] . "&search=$search'><img title='" . _('Delete') . "' src='./themes/" . $_SESSION[$guid]["gibbonThemeName"] . "/img/garbage.png'/></a>" ;
