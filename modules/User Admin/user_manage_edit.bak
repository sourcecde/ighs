...
if (isActionAccessible($guid, $connection2, "/modules/User Admin/user_manage_edit.php")==FALSE) {
	...
	$url=isset($_GET['url']) && $_GET['url']==1?$_SESSION[$guid]["absoluteURL"]."/index.php?q=/modules/".$_SESSION[$guid]["module"]."/user_manage.php":$_SESSION[$guid]["absoluteURL"]."/index.php?q=/modules/Students/student_view_details.php&gibbonPersonID=".$_GET['gibbonPersonID']."&subpage=Personal";
	print "<a href='".$url."' style='float:right;padding:5px;margin-top:-5px;background:seagreen;color:white;'>Back</a>";
	print "</div>" ;
...
<!---->						<h3><?php print _('Basic Information') ?></h3>
					...
					<?php  
					$admin=0;
					foreach($_SESSION[$guid]["gibbonRoleIDAll"] as $a){
						if($a[0]=='001' OR $a[0]=='030'){
								$admin=1;
								//echo $admin;
						}	
					}
...
<!-- -->				
					...
					if ($row["address1"]!="") {
						try {
							...
							$sqlAddress="SELECT gibbonPersonID, title, preferredName, surname, gibbonrole.category FROM gibbonperson JOIN gibbonrole ON (gibbonperson.gibbonRoleIDPrimary=gibbonrole.gibbonRoleID) WHERE status='Full' AND address1 LIKE :addressMatch AND NOT gibbonPersonID=:gibbonPersonID ORDER BY surname, preferredName" ;
			...
			?>
					...
					<tr>
						...
						<td class="right">
							<input name='religion' id='religion' type='text' value='<?php echo $row['religion']?>' style='width:300px'>
						</td>
					</tr>
					<!--  
					<tr>
					...
					<tr>
						<td> 
							...
							if ($_SESSION[$guid]["country"]=="") {
								...
								print "<b>" .'Aadhar Card Number'. "</b><br/>" ;
							}
							?>
						</td>
						<td class="right">
					...
					if ($parent) {
						...
						<tr>
						<td> 
							<b><?php print _('Annual Income (Rs.), i.e. currency') ?></b><br/>
						</td>
						<td class="right">
							<input name="annual_incode" id="annual_incode" maxlength=30 value="<?php print htmlPrep($row["annual_incode"]) ?>" type="text" style="width: 300px">
						</td>
					</tr>
						<?php
					}
			...
			?>		
					...
					if ($student) {
						...
						<tr>
							...
							<td class="right">
								<input name="account_number" id="account_number" maxlength=10 value="<?php print htmlprep(substr($row["account_number"],-5)) ?>" type="text" style="width: 300px">
							</td>
						</tr>
						<tr>
							<td> 
							...
							<td class="right">
								<input name="admission_number" id="admission_number" maxlength=15 value="<?php print htmlPrep($row["admission_number"]) ?>" type="text" style="width: 300px">
							</td>
						</tr>
						<?php
						try {
							$data=array("gibbonPersonID"=>$gibbonPersonID,"gibbonSchoolYearID"=>$_SESSION[$guid]['gibbonSchoolYearIDCurrent']); 
						...
						try {
							$data=array("gibbonYearGroupID"=>$enrolD['gibbonYearGroupID'],"gibbonSchoolYearID"=>$_SESSION[$guid]['gibbonSchoolYearIDCurrent']); 
							$sql="SELECT * FROM gibbonrollgroup WHERE gibbonYearGroupID=:gibbonYearGroupID AND gibbonSchoolYearID=:gibbonSchoolYearID" ;
							$result=$connection2->prepare($sql);
							$result->execute($data);
							$rollgroup=$result->fetchAll();
						}
						catch(PDOException $e) { 
							print "<div class='error'>" . $e->getMessage() . "</div>" ; 
						}
						try {
							$data=array("gibbonYearGroupID"=>$enrolD['gibbonYearGroupID']); 
						...
						<tr>
							...
							<td class="right">
								<input name="class" id="class" maxlength=10 value="<?php print htmlPrep($class["name"]) ?>" type="text" disabled="disabled" style="width: 300px">
							</td>
						</tr>
						<tr>
							<td> 
							...
							<td class="right">
								<select name='rollgroup' id='rollgroup' style="width: 90px">
								<?php foreach($rollgroup as $r){
									$selected=$r['gibbonRollGroupID']==$enrolD['gibbonRollGroupID']?"Selected":"";
									echo "<option value={$r['gibbonRollGroupID']} {$selected}>{$r['name']}</option>";
								}?>	
								</select>
							</td>
						</tr>
						<tr>
							...
							<td class="right">
								<input name="rollOrder" id="rollOrder" maxlength=10 value="<?php print htmlPrep($enrolD["rollOrder"]) ?>" type="text" style="width: 300px">
							</td>
						</tr>
					<?php }
					if ($student OR $staff) {
				...
				<!--	<tr>
							...
							<option value="No" <?php if($row["transport"]=='No'){?> selected="selected"<?php } ?>>No</option>
							<option value="Yes" <?php if($row["transport"]=='Yes'){?> selected="selected"<?php } ?>>Yes</option>
							</select>
					...
					*/
					...
					<tr>
						<td> 
							<b><?php print _('Boarder') ?></b><br/>
							<span style="font-size: 90%"></span>
						</td>
						<td class="right">
						<select name="boarder" id="boarder">
						<option value="N" <?php if($row["boarder"]=='N'){?> selected="selected"<?php } ?>>Non Boarder</option>
						<option value="Y" <?php if($row["boarder"]=='Y'){?> selected="selected"<?php } ?>>Boarder</option>
						<option value="D" <?php if($row["boarder"]=='D'){?> selected="selected"<?php } ?>>Day Boarder</option>
						</select>
