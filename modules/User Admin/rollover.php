...
if(!$_POST){
	...
	print "<h3>RE-ENROLMENT OF OLD STUDENTS</h3>";
						try {
							...
							$sqlReenrol="SELECT gibbonperson.gibbonPersonID, account_number, preferredName, `gibbonyeargroup`.`sequenceNumber`,`gibbonyeargroup`.`name` as class,`gibbonrollgroup`.`name` as section, `gibbonstudentenrolment`.`rollOrder`,gibbonRollGroupIDNext 
								...
								AND `gibbonperson`.`gibbonPersonID` NOT IN (SELECT `student_id` FROM `leftstudenttracker` WHERE 1)
                                                                AND `gibbonyeargroup`.gibbonYearGroupID < 7
			...
			$sqlSection="SELECT `gibbonRollGroupID`,`name` FROM `gibbonrollgroup` WHERE `gibbonSchoolYearID`=$nextYearID";
	...
	print "<input type='hidden' name='nextYearID' value='$nextYearID'>";
	...
	foreach($data as $d){
		...
		print "<tr>";	
			...
			print "<td>Section: {$d['section']} <br><span style='float:right'> Roll: {$d['rollOrder']}</span></td>";
				$classSelector="";
			...
			print "<td><select name='nextClass_$id'>$classSelector</select></td>";
				$sectionSelector="";
			...
			print "<td><select name='nextSection_$id'>$sectionSelector</select></td>";
		print "</tr>";	
	}
		print "<tr>";
