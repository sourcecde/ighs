...
if (isActionAccessible($guid, $connection2, "/modules/Students/report_student_dataUpdaterHistory.php")==FALSE) {
			...
			<tr>
				...
				<td class="right">
					<select name="Members[]" id="Members[]" multiple style="width: 302px; height: 150px">
						<optgroup label='--<?php print _('Students by Section') ?>--'>
							<?php
							try {
							...
							if(in_array($rowSelect["gibbonPersonID"], $membersarr))
								{
								...
								print "<option value='" . $rowSelect["gibbonPersonID"] . "'$selected>" . htmlPrep($rowSelect["name"]) . " - " . htmlPrep($rowSelect["officialName"]) . "</option>" ;
							}
							?>
						</optgroup>
						<optgroup label='--<?php print _('Students by Name') ?>--'>
							...
							try {
								$dataSelect=array("gibbonSchoolYearID"=>$_SESSION[$guid]["gibbonSchoolYearID"]); 
								$sqlSelect="SELECT gibbonperson.gibbonPersonID, preferredName, surname,officialName gibbonrollgroup.name AS name FROM gibbonperson, gibbonstudentenrolment, gibbonrollgroup WHERE gibbonperson.gibbonPersonID=gibbonstudentenrolment.gibbonPersonID AND gibbonstudentenrolment.gibbonRollGroupID=gibbonrollgroup.gibbonRollGroupID AND gibbonrollgroup.gibbonSchoolYearID=:gibbonSchoolYearID ORDER BY surname, preferredName" ;
								$resultSelect=$connection2->prepare($sqlSelect);
								$resultSelect->execute($dataSelect);
							}
							catch(PDOException $e) { }
							...
							if(in_array($rowSelect["gibbonPersonID"], $membersarr))
								...
								print "<option value='" . $rowSelect["gibbonPersonID"] . "'$selected>" . formatName("", htmlPrep($rowSelect["preferredName"]), htmlPrep($rowSelect["surname"]), "Student", true) . " (" . htmlPrep($rowSelect["name"]) . ")</option>" ;
	...
	if (count($choices)>0) {
		...
		try {
			...
			$sqlWhere=$sqlWhere . ")" ;
			$sql="SELECT surname, preferredName, gibbonperson.gibbonPersonID, gibbonrollgroup.name AS name,gibbonyeargroup.name as class FROM gibbonperson 
				...
				if ($personalFail OR $medicalFail OR $nonCompliant=="") {
					...
					print "<tr class=$rowNum>" ;
						...
						print "<td>" ;
							...
							//print htmlPrep($row["officialName"]) ;
						print "</td>" ;
						print "<td>" ;
							print $row["class"] ;
						print "</td>" ;
						print "<td>" ;
							print SectionFormater($row["name"]) ;
						print "</td>" ;
						print "<td>" ;
							print $personal ;
						print "</td>" ;
