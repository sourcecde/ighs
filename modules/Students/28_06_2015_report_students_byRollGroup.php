	...
	if ($gibbonRollGroupID!="") {
		...
		try {
			if ($gibbonRollGroupID=="*") {
				...
				WHERE status='Full' AND (dateStart IS NULL OR dateStart<='" . date("Y-m-d") . "') AND (dateEnd IS NULL  OR dateEnd>='" . date("Y-m-d") . "') AND gibbonstudentenrolment.gibbonSchoolYearID=:gibbonSchoolYearID ORDER BY gibbonstudentenrolment.rollOrder" ;
			}
			else {
				$data=array("gibbonRollGroupID"=>$gibbonRollGroupID); 
				$sql="SELECT gibbonperson.*,gibbonstudentenrolment.gibbonStudentEnrolmentID,gibbonyeargroup.name AS class,gibbonrollgroup.name AS section,gibbonstudentenrolment.rollOrder AS roll_number FROM gibbonperson 
				...
				JOIN gibbonrollgroup ON (gibbonstudentenrolment.gibbonRollGroupID=gibbonrollgroup.gibbonRollGroupID) 
				LEFT JOIN gibbonyeargroup ON gibbonstudentenrolment.gibbonYearGroupId=gibbonyeargroup.gibbonYearGroupId  
				WHERE status='Full' AND (dateStart IS NULL OR dateStart<='" . date("Y-m-d") . "') AND (dateEnd IS NULL  OR dateEnd>='" . date("Y-m-d") . "') AND gibbonstudentenrolment.gibbonRollGroupID=:gibbonRollGroupID ORDER BY gibbonstudentenrolment.rollOrder" ;
			}
			$result=$connection2->prepare($sql);
			$result->execute($data);
		}
