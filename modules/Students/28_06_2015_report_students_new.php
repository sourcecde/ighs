			...
			try {
				if ($type=="Date Range") {
					if ($ignoreEnrolment!="on") {
						...
						$sql="SELECT DISTINCT account_number,enrollment_date,gender,gibbonperson.gibbonPersonID,account_number,enrollment_date, surname, preferredName, username, dateStart,dob, lastSchool,gibbonyeargroup.name as class,gibbonrollgroup.name AS section,gibbonstudentenrolment.gibbonStudentEnrolmentID FROM gibbonperson 
...
LEFT JOIN gibbonrollgroup ON gibbonstudentenrolment.gibbonRollGroupID=gibbonrollgroup.gibbonRollGroupID  
						WHERE dateStart>=:startDateFrom AND dateStart<=:startDateTo AND status='Full' ORDER BY dateStart ASC" ;
						//echo $sql;
					}
					else {
						$data=array("startDateFrom"=>dateConvert($guid, $startDateFrom), "startDateTo"=>dateConvert($guid, $startDateTo)); 
						...
						LEFT JOIN gibbonyeargroup ON gibbonstudentenrolment.gibbonYearGroupId=gibbonyeargroup.gibbonYearGroupId
LEFT JOIN gibbonrollgroup ON gibbonstudentenrolment.gibbonRollGroupID=gibbonrollgroup.gibbonRollGroupID
						 WHERE dateStart>=:startDateFrom AND dateStart<=:startDateTo AND status='Full' ORDER BY dateStart,  preferredName" ;
						
					}
				}
				else if ($type=="Current School Year") {
					$data=array("gibbonSchoolYearID"=>$_SESSION[$guid]["gibbonSchoolYearID"]); 
					$sql="SELECT account_number,enrollment_date,gender,gibbonperson.gibbonPersonID,account_number,enrollment_date, surname, preferredName, gibbonrollgroup.nameShort AS rollGroup, username, dateStart, lastSchool,dob,gibbonyeargroup.name as class,gibbonrollgroup.name AS section,gibbonstudentenrolment.gibbonStudentEnrolmentID FROM gibbonperson 
					JOIN gibbonstudentenrolment ON (gibbonstudentenrolment.gibbonPersonID=gibbonperson.gibbonPersonID)
					LEFT JOIN gibbonyeargroup ON gibbonstudentenrolment.gibbonYearGroupId=gibbonyeargroup.gibbonYearGroupId 
					JOIN gibbonrollgroup ON (gibbonstudentenrolment.gibbonRollGroupID=gibbonrollgroup.gibbonRollGroupID) 
					WHERE gibbonstudentenrolment.gibbonSchoolYearID=:gibbonSchoolYearID AND status='Full' ORDER BY rollGroup,  preferredName" ;
				}
					...
					print "<table cellspacing='0' style='width: 100%'>" ;
						...
						while ($row=$result->fetch()) {
							...
							try {
								...
								$sql2="SELECT account_number,enrollment_date,gender,surname, preferredName, gibbonrollgroup.nameShort AS rollGroup, username FROM gibbonperson 
								JOIN gibbonstudentenrolment ON (gibbonstudentenrolment.gibbonPersonID=gibbonperson.gibbonPersonID) 
								JOIN gibbonrollgroup ON (gibbonstudentenrolment.gibbonRollGroupID=gibbonrollgroup.gibbonRollGroupID) 
								WHERE gibbonstudentenrolment.gibbonSchoolYearID=:gibbonSchoolYearID AND status='Full' AND gibbonperson.gibbonPersonID=:gibbonPersonID ORDER BY rollGroup, surname, preferredName" ;
