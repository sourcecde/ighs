...
if (isActionAccessible($guid, $connection2, "/modules/Students/report_transport_student.php")==FALSE) {
	...
	try {
		...
		$sql="SELECT gibbonperson.gibbonPersonID, transport, surname, preferredName,officialName, address1, address1District, address1Country, nameShort FROM gibbonperson JOIN gibbonstudentenrolment ON (gibbonperson.gibbonPersonID=gibbonstudentenrolment.gibbonPersonID) JOIN gibbonrollgroup ON (gibbonstudentenrolment.gibbonRollGroupID=gibbonrollgroup.gibbonRollGroupID) WHERE gibbonstudentenrolment.gibbonSchoolYearID=:gibbonSchoolYearID AND status='Full' AND (dateStart IS NULL OR dateStart<='" . date("Y-m-d") . "') AND (dateEnd IS NULL  OR dateEnd>='" . date("Y-m-d") . "') ORDER BY transport, surname, preferredName" ;
	...
	print "<table cellspacing='0' style='width: 100%'>" ;
		...
		while ($row=$result->fetch()) {
			...
			print "<tr class=$rowNum>" ;
				...
				print "</td>" ;
				print "<td>" ;
					//print formatName("", $row["officialName"], $row["surname"], "Student", true) ;
				...
				print "<td>" ;
					...
					while ($rowFamily=$resultFamily->fetch()) {
						...
						while ($rowFamily2=$resultFamily2->fetch()) {
							...
							print "<u>" . $rowFamily2["officialName"] . "</u><br/>" ;
