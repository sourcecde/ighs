			...
			<tr>
				<td class="right">
					<select style="width: 302px" name="gibbonRollGroupID">
						<?php
						print "<option value=''>Select Class & Section</option>" ;
						try {
				...
				<td class="right">
					<select style="width: 302px" name="gibbonSchoolYearID">
<?php					foreach($yearDB as $y){
							if(isset($_REQUEST["gibbonSchoolYearID"])){
								$s=$y['gibbonSchoolYearID']==$_GET["gibbonSchoolYearID"]?"selected = 'selected'":"";
							}
							else{
								$s=$y['gibbonSchoolYearID']==$_SESSION[$guid]["gibbonSchoolYearID"]?"selected = 'selected'":"";
							}
...
?>
	...
	if ($gibbonRollGroupID!="") {
		...
		try {
			...
			$sqlh="SELECT * FROM gibbonsetting WHERE gibbonSystemSettingsID='147'";
		    $resulth=$connection2->prepare($sqlh);
		    $resulth->execute();
		    $header2=$resulth->fetch();
		}
		catch(PDOException $e) { 
			print "<div class='error'>" . $e->getMessage() . "</div>" ; 
		}
