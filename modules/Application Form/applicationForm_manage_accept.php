<?php
/*
Lakshya, Flexible & Open School System
		...
		if ($result->rowCount()!=1) {
			...
			if ($step==1) {
					...
					<table class='smallIntBorder' cellspacing='0' style="width: 100%">	
						<tr>
							<td> 
								...
								?>
								<input <?php print $checkedStudent ?> type='checkbox' name='informStudent'/> <?php print _('Automatically inform <u>student</u> of their Lakshya login details by email?') ?><br/>
								<?php
								$checkedParents="" ;
								...
								?>
								<input <?php print $checkedParents ?> type='checkbox' name='informParents'/> <?php print _('Automatically inform <u>parents</u> of their Lakshya login details by email?') ?><br/>
								
								<br/>
								<i><u><?php print _('The system will perform the following actions:') ?></u></i><br/>
								<ol>
									<li><?php print _('Create a Lakshya user account for the student.') ?></li>
									<?php
									...
									if ($row["gibbonFamilyID"]!="") {
										print "<li>" . _('Link the student to their family (who are already in Lakshya).') . "</li>" ;
									}
									else {
								...
								<ol>
									...
									<li><?php print _('Create a note of the student\'s scholarship information outside of Lakshya.') ?></li>
				...
				if ($failStudent==TRUE) {
					...
					print "</h4>" ;
					print "<ul>" ;
						print "<li><b>PersonID</b>: $gibbonPersonID</li>" ;
					...
					if ($row["gibbonFamilyID"]!=0) {
						...
						try {
							...
							$sqlParents="SELECT * FROM gibbonfamilyadult WHERE gibbonFamilyID=:gibbonFamilyID" ; 
						...
						while ($rowParents=$resultParents->fetch()) {
							...
							if ($resultRelationship->rowCount()==1) {
								...
								try {
									$data=array("gibbonFamilyID"=>$row["gibbonFamilyID"], "gibbonFamilyAdultID"=>$rowParents["gibbonFamilyAdultID"], "gibbonPersonID"=>$gibbonPersonID); 
									$sql="SELECT * FROM gibbonfamilyrelationship WHERE gibbonFamilyID=:gibbonFamilyID AND gibbonFamilyAdultID=:gibbonFamilyAdultID AND gibbonPersonID=:gibbonPersonID" ;
									$result=$connection2->prepare($sql);
									$result->execute($data);
								}
								catch(PDOException $e) { 
								...
								if ($result->rowCount()==0) {
									try {
										$data=array("gibbonFamilyID"=>$row["gibbonFamilyID"], "gibbonFamilyAdultID"=>$rowParents["gibbonFamilyAdultID"], "gibbonPersonID"=>$gibbonPersonID, "relationship"=>$relationship); 
										$sql="INSERT INTO gibbonfamilyrelationship SET gibbonFamilyID=:gibbonFamilyID, gibbonFamilyAdultID=:gibbonFamilyAdultID, gibbonPersonID=:gibbonPersonID, relationship=:relationship" ;
										$result=$connection2->prepare($sql);
										$result->execute($data);
									}
									catch(PDOException $e) { 
						...
						if ($lock==true) {
							...
							if ($gotAI==true) {
								...
								//$familyName=$row["parent1preferredName"] . " " . $row["parent1surname"] ;
								 $familyName=$row["parent2preferredName"];
								if ($row["parent1preferredName"]!="") {
									//$familyName.=" & " . $row["parent2preferredName"] . " " . $row["parent2surname"] ; 
									$familyName.=" & " . $row["parent1preferredName"]; 
								}
								$nameAddress="" ;
								...
								if ($row["parent1surname"]==$row["parent2surname"] AND $row["parent2preferredName"]!="" AND $row["parent2title"]!="") {
									$nameAddress=$row["parent2title"] . "& " . $row["parent1title"] . $row["parent2surname"] ;
								}
						...
						if ($failFamily==TRUE) {
							...
							//CREATE PARENT 1
							$failParent1=TRUE ;
							if ($row["parent1gibbonFamilyAdultID"]!="") {/*
								$gibbonPersonIDParent1=$row["parent1gibbonPersonID"];
								print "<h4>" ;
								print "Parent 1" ;
								print "</h4>" ;
								print "<ul>" ;
									print "<li>" . _('Parent 1 already exists in Lakshya, and so does not need a new account.') . "</li>" ;
								...
								$lock=true ;
								try {
									$sql="LOCK TABLES gibbonfamilyadult WRITE" ;
									$result=$connection2->query($sql);   
								}
								catch(PDOException $e) { 
									$lock=false ;
								...
								if ($lock==true) {
									$gotAI=true ;
									try {
										$sqlAI="SHOW TABLE STATUS LIKE 'gibbonfamilyadult'";
										$resultAI=$connection2->query($sqlAI);   
									}
									catch(PDOException $e) { 
										$gotAI=false ;
									...
									if ($gotAI==true) {
										$rowAI=$resultAI->fetch();
										$gibbonFamilyAdultID=str_pad($rowAI['Auto_increment'], 10, "0", STR_PAD_LEFT) ;
										$parent1gibbonFamilyAdultID=$gibbonFamilyAdultID;
										//Set username & password
								...
								try {
									...
									if ($gibbonFamilyID!="") {
										...
										if ($resultFamily->rowCount()==1) {
											...
											if ($familyName!=""){
												$insertOK=true ;
												$officialName=$row['parent1title'].$row['parent1firstName']." ".$row['parent1surname'];
												try {
													$data=array("gibbonPersonID"=>$gibbonPersonID, "gibbonFamilyID"=>$gibbonFamilyID,"officialName"=>$officialName, "nationalIDCardNumber"=>$row["parent1nationalIDCardNumber"], "email"=>$row["parent1email"], "phone1Type"=>$row["parent1phone1Type"],"phone1CountryCode"=>$row["parent1phone1CountryCode"],"phone1"=>$row["parent1phone1"],"phone2Type"=>$row["parent1phone2Type"],"phone2CountryCode"=>$row["parent1phone2CountryCode"],"phone2"=>$row["parent1phone2"], "profession"=>$row["parent1profession"], "employer"=>$row["parent1employer"]); 
													$sql="INSERT INTO gibbonfamilyadult SET gibbonPersonID=:gibbonPersonID, gibbonFamilyID=:gibbonFamilyID, contactPriority=2, contactCall='Y', contactSMS='Y', contactEmail='Y', contactMail='Y', officialName=:officialName,nationalIDCardNumber=:nationalIDCardNumber,email=:email,phone1Type=:phone1Type,phone1CountryCode=:phone1CountryCode,phone1=:phone1,phone2Type=:phone2Type,phone2CountryCode=:phone2CountryCode,phone2=:phone2,profession=:profession,employer=:employer" ;
													$result=$connection2->prepare($sql);
													//echo $sql;
													$result->execute($data);
												}
										...
										//Set parent relationship
										try {
											$data=array("gibbonFamilyID"=>$gibbonFamilyID, "gibbonFamilyAdultID"=>$gibbonFamilyAdultID, "gibbonPersonID"=>$gibbonPersonID, "relationship"=>$row["parent1relationship"]); 
											$sql="INSERT INTO gibbonfamilyrelationship SET gibbonFamilyID=:gibbonFamilyID, gibbonFamilyAdultID=:gibbonFamilyAdultID, gibbonPersonID=:gibbonPersonID, relationship=:relationship" ;
											//$sql="INSERT INTO gibbonfamilyrelationship SET gibbonFamilyID=:gibbonFamilyID, gibbonPersonID1=:gibbonPersonID1, gibbonPersonID2=:gibbonPersonID2, relationship=:relationship" ;
											$result=$connection2->prepare($sql);
											$result->execute($data);
							...
							if ($row["parent2preferredName"]!="") {
								...
								if ($lock==true) {
									...
									if ($gotAI==true) {
										$rowAI=$resultAI->fetch();
										$gibbonFamilyAdultID=str_pad($rowAI['Auto_increment'], 10, "0", STR_PAD_LEFT) ;
										$parent2gibbonFamilyAdultID=$gibbonFamilyAdultID;
									//echo "HULULU";
										//Set username & password
								...
								/*if ($failParent2==TRUE) {
									...
									if ($gibbonFamilyID!="") {
										...
										if ($resultFamily->rowCount()==1) {
											...
											if ($familyName!=""){
												$insertOK=true ;
												$officialName=$row['parent2title']." ".$row['parent2firstName']." ".$row['parent2surname'];
												try {
													$data=array("gibbonPersonID"=>$gibbonPersonID, "gibbonFamilyID"=>$gibbonFamilyID,"officialName"=>$officialName, "nationalIDCardNumber"=>$row["parent2nationalIDCardNumber"], "email"=>$row["parent2email"], "phone1Type"=>$row["parent2phone1Type"],"phone1CountryCode"=>$row["parent2phone1CountryCode"],"phone1"=>$row["parent2phone1"],"phone2Type"=>$row["parent2phone2Type"],"phone2CountryCode"=>$row["parent2phone2CountryCode"],"phone2"=>$row["parent2phone2"], "profession"=>$row["parent2profession"], "employer"=>$row["parent2employer"]); 
													$sql="INSERT INTO gibbonfamilyadult SET gibbonPersonID=:gibbonPersonID, gibbonFamilyID=:gibbonFamilyID, contactPriority=1, contactCall='Y', contactSMS='Y', contactEmail='Y', contactMail='Y', officialName=:officialName,nationalIDCardNumber=:nationalIDCardNumber,email=:email,phone1Type=:phone1Type,phone1CountryCode=:phone1CountryCode,phone1=:phone1,phone2Type=:phone2Type,phone2CountryCode=:phone2CountryCode,phone2=:phone2,profession=:profession,employer=:employer" ;	
													$result=$connection2->prepare($sql);
													$result->execute($data);
												}
												catch(PDOException $e) { 
										...
										//Set parent relationship
										try {
											$data=array("gibbonFamilyID"=>$gibbonFamilyID, "gibbonFamilyAdultID"=>$gibbonFamilyAdultID, "gibbonPersonID"=>$gibbonPersonID, "relationship"=>$row["parent2relationship"]); 
											$sql="INSERT INTO gibbonfamilyrelationship SET gibbonFamilyID=:gibbonFamilyID, gibbonFamilyAdultID=:gibbonFamilyAdultID, gibbonPersonID=:gibbonPersonID, relationship=:relationship" ;
											$result=$connection2->prepare($sql);
											$result->execute($data);
										}
					...
					try {
						$data=array("gibbonApplicationFormID"=>$gibbonApplicationFormID); 
						$sql="UPDATE gibbonapplicationform SET status='Accepted',gibbonFamilyID=$gibbonFamilyID,parent1gibbonFamilyAdultID=$parent1gibbonFamilyAdultID,parent2gibbonFamilyAdultID=$parent2gibbonFamilyAdultID WHERE gibbonApplicationFormID=:gibbonApplicationFormID" ;
						$result=$connection2->prepare($sql);
						$result->execute($data);
					}
					catch(PDOException $e) { 
					...
					if ($failStatus==true) {
						...
						print _("Applicant has been successfully accepted.") . " <i><u>" . _('You may wish to now do the following:') . "</u></i><br/>" ;
						print "<ol>" ;
							...
							print "<li>" . _('Create a note of the student\'s scholarship information outside of Lakshya.') . "</li>" ;
							print "<li>" . _('Create a timetable for the student.') . "</li>" ;
							print "<li>" . _('Inform the student and their parents of their Lakshya login details (if this was not done automatically).') . "</li>" ;
						print "</ol>" ;
						print "</div>" ;
						print "<div style='text-align:center;'><br/><a href='".$_SESSION[$guid]["absoluteURL"]."/index.php?q=/modules/User%20Admin/applicationForm_manage.php' style='background: seagreen;padding: 6px;color: white;'>Back</a></div>";
					}
				}
			}
		}
