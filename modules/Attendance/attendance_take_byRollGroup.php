...
if (isActionAccessible($guid, $connection2, "/modules/Attendance/attendance_take_byRollGroup.php")==FALSE) {
	...
	print "<div class='trailHead'><a href='" . $_SESSION[$guid]["absoluteURL"] . "'>" . _("Home") . "</a> > <a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . getModuleName($_GET["q"]) . "/" . getModuleEntry($_GET["q"], $connection2, $guid) . "'>" . _(getModuleName($_GET["q"])) . "</a> > </div><div class='trailEnd'>" . _('Take Attendance by Section') . "</div>" ;
	print "</div>" ;
	...
	<form method="get" action="<?php print $_SESSION[$guid]["absoluteURL"]?>/index.php">
		<table class='smallIntBorder' cellspacing='0' style="width: 100%">	
			...
			<tr>
				...
				<td class="right">
					<select style="width: 302px" name="gibbonRollGroupID" id="gibbonRollGroupID">
						<?php
						print "<option value=''> - SELECT - </option>" ;
						try {
							$dataSelect=array("gibbonSchoolYearID"=>$_SESSION[$guid]["gibbonSchoolYearID"]); 
							$sqlSelect="SELECT `gibbonRollGroupID`,concat(yg.name, '-', rg.name) as name 								
							FROM gibbonrollgroup rg JOIN gibbonyeargroup yg ON rg.gibbonYearGroupID=yg.gibbonYearGroupID 								WHERE gibbonSchoolYearID=:gibbonSchoolYearID ORDER BY name" ;
			...
			<tr>
			<td><b>With Left Student</b></td>
			<td class="right">
			<input type="checkbox" name="left_student" id="left_student" value="1" <?php if(isset($_REQUEST['left_student'])){?> checked="checked" <?php } ?>>
			</td>
			</tr>
			<tr>
				<td colspan=2 class="right">
					...
					<?php if(isset($_REQUEST['currentDate'])){?>
					<input type="button" name="print_atendence" id="print_atendence" value="Print">
					<?php } ?>
					<input type="hidden" name="print_page_url" id="print_page_url" value="<?php echo $_SESSION[$guid]["absoluteURL"] . "/modules/Attendance/print_attendence_report.php";?>">
				</td>
			</tr>
		</table>
	</form>
		...
		if ($currentDate>$today) {
			...
			if (false) {
				...
				try {
					...
					$sqlLog="SELECT * FROM gibbonattendancelogrollgroup, gibbonperson WHERE gibbonattendancelogrollgroup.gibbonPersonIDTaker=gibbonperson.gibbonPersonID AND gibbonattendancelogrollgroup.gibbonRollGroupID=:gibbonRollGroupID AND date LIKE :date ORDER BY timestampTaken" ;
				...
				try {
					...
					{
						$sqlRollGroup="SELECT * FROM gibbonstudentenrolment INNER JOIN gibbonperson ON gibbonstudentenrolment.gibbonPersonID=gibbonperson.gibbonPersonID WHERE gibbonstudentenrolment.gibbonRollGroupID=:gibbonRollGroupID AND status='Full' AND gibbonperson.dateStart<='".$currentDate."'  ORDER BY rollOrder" ;
						
					}
					else 
					{
						$sqlRollGroup="SELECT * FROM gibbonstudentenrolment INNER JOIN gibbonperson ON gibbonstudentenrolment.gibbonPersonID=gibbonperson.gibbonPersonID WHERE gibbonstudentenrolment.gibbonRollGroupID=:gibbonRollGroupID AND status='Full' AND gibbonperson.dateStart<='".$currentDate."' AND  gibbonperson.dateEnd IS NULL ORDER BY rollOrder" ;
					...
					print "<form method='post' action='" . $_SESSION[$guid]["absoluteURL"] . "/modules/" . $_SESSION[$guid]["module"] . "/attendance_take_byRollGroupProcess.php'>" ;
						...
						while ($rowRollGroup=$resultRollGroup->fetch()) {
							...
							if ($rowLog["type"]=="Absent") {
								...
								print "<div style='padding-top: 5px'><b><span style='color:red'>Roll No: " . $rowRollGroup["rollOrder"] . "</span><b></div><br/>" ;
