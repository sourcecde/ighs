...
if (isActionAccessible($guid, $connection2, "/modules/Attendance/attendance_take_byRollGroup.php")==FALSE) {
	...
	//Select Section
	$sqlselect="SELECT `gibbonRollGroupID`, concat(`gibbonyeargroup`.`name`,'-',`gibbonrollgroup`.`name`) as name 				FROM `gibbonrollgroup` 				JOIN gibbonyeargroup  ON gibbonrollgroup.gibbonYearGroupID=gibbonyeargroup.gibbonYearGroupID 				LEFT JOIN `gibbonschoolyear` ON `gibbonschoolyear`.`gibbonSchoolYearID`=`gibbonrollgroup`.`gibbonSchoolYearID` 				WHERE ";
