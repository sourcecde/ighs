				...
				<td class="right">
					...
					<select style="width: 302px" name="gibbonPersonID">
						...
						//	$sqlSelect="SELECT * FROM gibbonperson JOIN gibbonstudentenrolment ON (gibbonperson.gibbonPersonID=gibbonstudentenrolment.gibbonPersonID) JOIN gibbonrollgroup ON (gibbonstudentenrolment.gibbonRollGroupID=gibbonrollgroup.gibbonRollGroupID) WHERE gibbonrollgroup.gibbonSchoolYearID=:gibbonSchoolYearID AND status='Full' AND (dateStart IS NULL OR dateStart<='" . date("Y-m-d") . "') AND (dateEnd IS NULL  OR dateEnd>='" . date("Y-m-d") . "') ORDER BY surname, preferredName" ;
							$sqlSelect="SELECT * FROM gibbonperson JOIN gibbonstudentenrolment ON (gibbonperson.gibbonPersonID=gibbonstudentenrolment.gibbonPersonID) JOIN gibbonrollgroup ON (gibbonstudentenrolment.gibbonRollGroupID=gibbonrollgroup.gibbonRollGroupID) WHERE gibbonrollgroup.gibbonSchoolYearID=:gibbonSchoolYearID AND status='Full' AND (dateStart IS NULL OR dateStart<='" . date("Y-m-d") . "') AND (dateEnd IS NULL  OR dateEnd>='" . date("Y-m-d") . "') ORDER BY officialName" ;
						...
						while ($rowSelect=$resultSelect->fetch()) {
							if ($gibbonPersonID==$rowSelect["gibbonPersonID"]) {
								print "<option selected value='" . $rowSelect["gibbonPersonID"] . "'>" . formatName("", htmlPrep($rowSelect["preferredName"]), htmlPrep($rowSelect["surname"]), "Student", true) . " (" . htmlPrep($rowSelect["nameShort"]) . ") - " . htmlPrep(substr($rowSelect["account_number"], 5)) . "</option>" ;
                                                              //  print "<option  value='" . $rowSelect["gibbonPersonID"] . "' selected='selected'>" . htmlPrep($rowSelect["officialName"]). " (" . htmlPrep($rowSelect["nameShort"]) . ")"."</option>" ;							
							}
							else {
								print "<option value='" . $rowSelect["gibbonPersonID"] . "'>" . formatName("", htmlPrep($rowSelect["preferredName"]), htmlPrep($rowSelect["surname"]), "Student", true) . " (" . htmlPrep($rowSelect["nameShort"]) . ") - " . htmlPrep(substr($rowSelect["account_number"], 5)) . "</option>" ;
								//print "<option selected value='" . $rowSelect["gibbonPersonID"] . "'>" . htmlPrep($rowSelect["officialName"]). " (" . htmlPrep($rowSelect["nameShort"]) . ")"."</option>" ;
							}
						}
						?>				
					</select>
