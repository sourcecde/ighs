...
//VALIDATE LOGIN INFORMATION
	...
	try {
		...
		$sql="SELECT * FROM gibbonperson WHERE ((username=:username) AND (status='Full') AND (canLogin='Y') AND (gibbonRoleIDPrimary=003))" ;
			...
			if ($passwordTest!=true) {
				...
				if ($row["gibbonRoleIDPrimary"]=="" OR count(getRoleList($row["gibbonRoleIDAll"], $connection2))==0) {
					...
					if ($_POST["gibbonSchoolYearID"]!=$_SESSION[$guid]["gibbonSchoolYearID"]) {
						try {
							$dataYear=array("gibbonSchoolYearID"=>$_SESSION[$guid]["gibbonSchoolYearID"]); 
							echo $sqlYear="SELECT * FROM gibbonschoolyear WHERE gibbonSchoolYearID=:gibbonSchoolYearID" ;
