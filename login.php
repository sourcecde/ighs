		...
		if ($row["failCount"]>=10) {
			...
			if ($passwordTest!=true) {
				...
				if ($row["gibbonRoleIDPrimary"]=="" OR count(getRoleList($row["gibbonRoleIDAll"], $connection2))==0) {
					...
					$otpLogin=getSettingByScope($connection2, "System", "otpLogin") ;
					/* Non-OTP Login*/
					#region  Non-OTP Login	
					if($otpLogin!='Y'){
					...
					#region OTP Login
						...
						$message="Your OTP to login is : ".$otp;
						$phoneNo=$row["phone1"];
						if(SendSMSCommon($connection2, $message, $phoneNo))
						{
							$_SESSION[$guid]["login_otp"]=array("OTP"=>$otp, "attempt"=>0, "personId"=>$row["gibbonPersonID"],"gibboni18nID"=>$_POST["gibboni18nID"]);
							$alert="OTP has been sent to your  registered mobile number ".substr($phoneNo,0,2)."******".substr($phoneNo,8,2).".";
							$URL.="?loginReturn=fail20&message=$alert" ;
							header("Location: {$URL}");
						}
						else
						{
							$URL.="?loginReturn=fail23" ;
							header("Location: {$URL}");
						}
					}
					#end
