...
function getMinorLinks($connection2, $guid, $cacheLoad) {
	...
	if (isset($_SESSION[$guid]["username"])) {
		...
		$return.="<img src='{$_SESSION[$guid]["absoluteURL"]}/themes/Default/img/schoollogo.png' alt='' />" ;
		$return.=$_SESSION[$guid]["preferredName"] . " . " ;
		$return.="<a href='./logout.php'>" . _("Logout") . "</a> . <a href='./index.php?q=preferences.php'>" . _('Change Password') . "</a><div class='paneltopleft'>" ;
		...
		if (isset($_SESSION[$guid]["likeCount"])) {
			if ($_SESSION[$guid]["likeCount"]>0) {
				$return.="<a title='" . substr($_SESSION[$guid]["likeCountTitle"],0,-2) . "' href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=stars.php'>" . $_SESSION[$guid]["likeCount"] . " x <span class='likeoffon on'>&#xf005;</span></a>" ;
			}
			else {
				$return.="" . $_SESSION[$guid]["likeCount"] . " x <span class='likeoffon off'>&#xf005;</span>" ; 
			}
		}
		...
		if ($resultNotifications->rowCount()>0) {
			$return.=" . <a title='" . _('Notifications') . "' href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=notifications.php'>" . $resultNotifications->rowCount() . " x " . "<span class='nofify on'>&#xf0f3;</span></a>";
		}
		else {
			$return.=" . 0 x " . "<span class='nofify off'>&#xf0f3;</span>" ;
		}
		...
		if (isActionAccessible($guid, $connection2, "/modules/Messenger/messageWall_view.php")) {
			...
			if (isset($_SESSION[$guid]["messageWallCount"])==FALSE) {
				$return.=" . 0 x <span class='message off'>&#xf0e0;</span>" ;
				$return.="<span class='schoollogo'><img src='" . $_SESSION[$guid]["absoluteURL"] . "/themes/" . $_SESSION[$guid]["gibbonThemeName"] . "/img/schoollogo.png' alt=''></span>" ;
			}
			else {
				if ($_SESSION[$guid]["messageWallCount"]<1) {
					$return.=" . 0 x <span class='message off'>&#xf0e0;</span>" ;
					$return.="</div><span class='schoollogo'><img src='" . $_SESSION[$guid]["absoluteURL"] . "/themes/" . $_SESSION[$guid]["gibbonThemeName"] . "/img/schoollogo.png' alt=''></span>" ;
				}
				else {
					$return.=" . <a title='" . _('Message Wall') . "'class='thickbox' href='$URL'>" . $_SESSION[$guid]["messageWallCount"] . " x <span class='message on'>&#xf0e0;</span></a>" ;
...
function getStudentFastFinder($connection2, $guid) {
	...
	$output.="<div id='findStaffStudents'>" ;
		...
		if ($studentHighestAction=="View Student Profile_full" OR $staffIsAccessible==TRUE) {
			...
			$output.="<style>" ;
				$output.="ul.token-input-list-facebook { width: 251px; float: left; height: 28px!important; }" ;
				$output.="div.token-input-dropdown-facebook { width: 251px }";
			$output.="</style>" ;
			$output.="<div>" ;
					...
					$output.="<table class='smallIntBorder' cellspacing='0' style='width: 100%; margin: 0px 0px; opacity: 0.8'>" ;	
						...
						$output.="<tr>" ;
							...
							$output.="<td class='right' style='vertical-align: top; border: none'>" ;
								$output.="<input class='findbtn' type='submit' value='" . _('Go') . "'>" ;
							$output.="</td>" ;
						$output.="</tr>" ;
						if (getRoleCategory($_SESSION[$guid]["gibbonRoleIDCurrent"], $connection2)=="Staff") {
							$output.="<tr>" ;
								$output.="<td class='studenttd' style='vertical-align: top' colspan=2>" ;
									$output.="<div class='totalstudent'>" . _('Total Student Enrolment:') . " " . $studentCount . "</div>" ;
								$output.="</td>" ;
							$output.="</tr>" ;
						}
					$output.="</table>" ;
...
function sidebar($connection2, $guid) {
	...
	if (!($loginReturn=="")) {
		...
		else if ($loginReturn=="fail20") {
			if (isset($_GET["message"])) {
				$loginReturnMessage=$_GET["message"];	
			}
			else {
				$loginReturnMessage=_("OTP has been sent successfully.") ;	 
			}
		}
		else if ($loginReturn=="fail21") {
			$loginReturnMessage=_("Please enter correct OTP.") ;	
		}
		else if ($loginReturn=="fail22") {
			$loginReturnMessage=_("Too many failed attept: please login again.") ;	
		}
		else if ($loginReturn=="fail23") {
			$loginReturnMessage=_("We are facing technical error. Please try after sometime.") ;	
		}
	...
	if ((isset($_SESSION[$guid]["username"])==FALSE) && (isset($_SESSION[$guid]["email"])==FALSE)) {
		if($googleOAuth=="Y") {
			...
			if(isset($_SESSION[$guid]["login_otp"])){
				if (isset($_SESSION[$guid]["pageLoads"])) {
					$_SESSION[$guid]["pageLoads"]-- ;
				}
			?>
				<div class="userlogo"><img src="<?php print $_SESSION[$guid]["absoluteURL"] ?>/themes/Default/img/schoollogo.png" alt="" /></div> 
				<h2>Verify OTP</h2>
				<form name="otpForm" method="post" action="./verifyotp.php">
				<table class='noIntBorder' cellspacing='0' style="width: 100%; margin: 0px 0px">	
					<tr>
						<td colspan="2">
							<input name="otp" id="otp" maxlength=6 type="text" required>
						</td>
					</tr>
					
					<tr>
						<td class="right" colspan="2">
							<a id="loginBackBtn" href="./logout.php">Logout</a>
			...
			else if ((isset($_SESSION[$guid]["username"])==FALSE)){ // If Google Auth set to No make sure login screen not visible when logged in
			?>
            <div class="userlogo"><img src="<?php print $_SESSION[$guid]["absoluteURL"] ?>/themes/Default/img/schoollogo.png" alt="" /></div>
			<h2>
				<?php print _("Login") ; ?>
			...
			<form name="loginForm" method="post" action="./login.php?<?php if (isset($_GET["q"])) { print "q=" . $_GET["q"] ; } ?>">
				<table class='noIntBorder' cellspacing='0' style="width: 100%; margin: 0px 0px">	
					<tr>
						...
						<td colspan="2"><div class="fieldwrap">
							<input name="username" id="username" maxlength=20 type="text">
					...
					<tr>
						...
						<td colspan="2"><div class="fieldwrap password">
					...
					<tr class='schoolYear' id='schoolYear'>
						...
						<td class="right">
							<select name="gibbonSchoolYearID" id="gibbonSchoolYearID">
								<?php
								try {
									$dataSelect=array(); 
					...
					<tr class='language' id='language'>
						...
						<td class="right">
							<select name="gibboni18nID" id="gibboni18nID">
								<?php
								try {
									$dataSelect=array(); 
					...
					<tr>
						...
						<td colspan="2" class="forgotlinks">
							<?php
							print "<script type='text/javascript'>" ;	
								print "$(document).ready(function(){" ;
									...
									print "\$(\".schoolYear\").slideToggle(400);" ;
									print "\$(\".language\").slideToggle(400);" ;
									print "});" ;
								print "});" ;
							print "</script>" ;
							?>
							<span style='font-size: 10px'><a class='show_hide' onclick='false' href='javascript:void(0);'><?php print _("Options") ; ?></a> . <a href="<?php print $_SESSION[$guid]["absoluteURL"]?>/index.php?q=passwordReset.php"><?php print _("Forgot Password?") ; ?></a></span>
						</td>
					</tr>
					<tr>
						<td class="right" colspan=2>
							<a id="loginBackBtn" href="http://ighs.in/lakshya/index.html"><-Back</a>
				...
				while ($row=$result->fetch()) {
					...
					if ($currentName!=$lastName) {
						if ($currentCategory!=$lastCategory) {
							...
							$output.="<li class='haschild'><h3>" . _($currentCategory) . "</h3>" ;
							$output.="<ul>" ;
							$output.="<li><a class='profileviewlink' $style href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . $row["moduleName"] . "/" . $row["entryURL"] . "'>" . _($currentName) . "</a></li>" ;
						}
						else {
...
function getUserPhoto($guid, $path, $size) {
	...
	//if ($path=="" OR file_exists($_SESSION[$guid]["absolutePath"] . "/" . $path)==FALSE) {    
	if ($path=="" OR file_exists( $path)==FALSE) {    
		...
		$output="<img $sizeStyle class='user' src='".$path."'/><br/>" ;
	}
	return $output ;
}
...
{
	return substr($section, -5);
}
//Convert Number into Word. #copied_Nazmul
...
function SendSMSCommon($connection1, $message_data, $phoneNo){
